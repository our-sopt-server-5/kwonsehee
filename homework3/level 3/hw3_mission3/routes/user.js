...
router.post('/signup', async (req, res) => {
    ...
    // const salt = crypto.randomBytes(8).toString('hex');
    // const hash = await crypto.pbkdf2Sync(password, user[0].salt);
...
router.post('/signin', async (req, res) => {
    ...
    // const hash = await crypto.pbkdf2Sync(password, user[0].salt, 100000,64,'sha512');
    // if(hash !== user.hash){
    //     return res.status(statusCode.BAD_REQUEST)
    //     .send((util.fail(statusCode.BAD_REQUEST, resMessage.MISS_MATCH_PW)));
    // }
    // 비밀번호 확인 - 없다면 Miss match password 반환
    const hash = await encrypt.encryptWithSalt(password, user.salt);
